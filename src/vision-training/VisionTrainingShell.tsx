...
import PlayerNotesScreen from "./PlayerNotesScreen";
...
function parsePath(): Route {
  ...
  const notesMatch = path.match(/^\/vision-training\/session\/([^/]+)\/notes$/);
  if (notesMatch) return { view: "notes", sessionId: notesMatch[1] };
...
export default function VisionTrainingShell() {
  ...
  if (route.view === "notes") return <PlayerNotesScreen sessionId={route.sessionId} />;
