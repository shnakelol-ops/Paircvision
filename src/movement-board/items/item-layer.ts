...
export function createTrainingItemLayer(options: CreateTrainingItemLayerOptions): TrainingItemLayerHandle {
  ...
  const normToWorld = (nx: number, ny: number) => ({ x: (nx / 100) * WORLD_W, y: (ny / 100) * WORLD_H });
  const worldToNorm = (wx: number, wy: number) => ({ x: (wx / WORLD_W) * 100, y: (wy / WORLD_H) * 100 });
  ...
  const emitItems = () => options.onItemsChange?.(items.map(cloneItem));
  ...
  const drawItemGraphic = (graphic: Graphics, item: TacticalTrainingItem) => {
    ...
    if (item.type === "cone") {
      ...
      graphic.poly([-ITEM_HALF_SIZE, ITEM_HALF_SIZE, 0, -ITEM_HALF_SIZE, ITEM_HALF_SIZE, ITEM_HALF_SIZE]).fill(0xf59e0b).stroke({ color: 0xb45309, width: 0.45 });
      return;
    }
    if (item.type === "pole") {
      graphic.ellipse(0, ITEM_HALF_SIZE * 0.78, ITEM_HALF_SIZE * 0.62, ITEM_HALF_SIZE * 0.22).fill({ color: shadow, alpha: 0.16 });
      graphic.roundRect(-0.45, -ITEM_HALF_SIZE * 1.15, 0.9, ITEM_HALF_SIZE * 2.3, 0.35).fill(0xfde68a).stroke({ color: 0x92400e, width: 0.32 });
      return;
    }
  ...
  const drawSelection = (selection: Graphics, selected: boolean) => {
    selection.clear();
    if (selected) selection.circle(0, 0, ITEM_HALF_SIZE * 1.55).stroke({ color: 0x7dd3fc, alpha: 0.94, width: 0.42 });
  };
  ...
  const selectItem = (id: string | null) => {
    ...
    selectedItemId = id;
    for (const visual of visuals.values()) drawSelection(visual.selection, visual.item.id === selectedItemId);
  ...
  const onItemPointerDown = (visual: TrainingItemVisual, event: unknown) => {
    ...
    if (pointerId == null || !worldPt) return;
    const normPt = worldToNorm(worldPt.x, worldPt.y);
    activeDrag = { itemId: visual.item.id, pointerId, startNormX: visual.item.x, startNormY: visual.item.y, startPtrNormX: normPt.x, startPtrNormY: normPt.y };
  };
  ...
  const onGlobalPointerMove = (event: unknown) => {
    ...
    if (itemIndex < 0) { activeDrag = null; return; }
    const updated = { ...items[itemIndex]!, x: clampNorm(activeDrag.startNormX + normPt.x - activeDrag.startPtrNormX), y: clampNorm(activeDrag.startNormY + normPt.y - activeDrag.startPtrNormY) };
    items = items.map((item, index) => (index === itemIndex ? updated : item));
    const visual = visuals.get(updated.id);
    if (visual) { visual.item = updated; syncVisual(visual); }
  };
  ...
  const rebuild = (nextItems: readonly TacticalTrainingItem[]) => {
    ...
    for (const item of items) visuals.set(item.id, createVisual(item));
  ...
  return {
    setItems: (nextItems) => { activeDrag = null; rebuild(nextItems); },
    getItems: () => items.map(cloneItem),
    setSelectedItemId: (id) => selectItem(id),
    ...
    syncToMapper: () => { for (const visual of visuals.values()) syncVisual(visual); },
    destroy: () => {
