...
function parseStoredLoggedMatchEvent(input: unknown): LoggedMatchEvent | null {
  ...
  // Structural check only: the kind field must be present and be a string.
  // Whether it's one of the *current* MATCH_EVENT_KINDS is deliberately NOT
  // checked here — that's an additive-schema concern handled by callers
  // (see isKnownEventKind below), so a stored/imported event with a kind
  // this build doesn't recognise yet is never treated as malformed.
