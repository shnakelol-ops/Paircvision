 ...
 *
 * Keyed by playerId (globally unique across both squads — see
 * ProTaggerSession's newId()), so Home #12 and Away #12 never collide even
 * though buildDisciplineStatusMap scans both teams' events in one pass.
 *
 ...
 * time, so it is not a safe bridge either — see the audit in this PR).
 *
