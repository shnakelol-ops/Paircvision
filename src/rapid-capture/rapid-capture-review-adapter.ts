 ...
 * Mirrors StatsModeSurface.tsx's handleGenerateIntelligencePack exactly: a
 * "Half Time" pack is built from first-half events only (score included) —
 * only "Full Time" sees the whole match. Getting this wrong would silently
 * diverge from what Match Stats produces for the same underlying events.
 */
export function rapidMatchToIntelligencePackInput(
  match: RapidSavedMatch,
  stageLabel: IntelligencePackStage,
): IntelligencePackInput {
  const packEvents = stageLabel === "Half Time" ? match.events.filter((e) => e.half === 1) : match.events;
  const scoreboard = computeRapidScoreboard(packEvents);
  return {
    stageLabel,
    ...
    events: packEvents.map(normalizeRapidEventForReports),
  };
}
