...
describe("rapidMatchToIntelligencePackInput", () => {
  ...
  describe("Half Time stage mirrors StatsModeSurface.tsx's handleGenerateIntelligencePack exactly", () => {
    // StatsModeSurface.tsx filters to `period === "1H"` and computes score
    // from that filtered set before calling buildIntelligencePack for a
    // "Half Time" pack — only "Full Time" sees every event. Diverging from
    // this would silently produce a different pack than Match Stats for
    // the same underlying events.
    const firstHalfGoal = buildCapturedEvent({ kind: "GOAL", nx: 0.9, ny: 0.5, half: 1, timestamp: 10, teamSide: "FOR" });
    const secondHalfPoint = buildCapturedEvent({ kind: "POINT", nx: 0.9, ny: 0.5, half: 2, timestamp: 900, teamSide: "OPP" });
    const match = buildMatch({ events: [firstHalfGoal, secondHalfPoint] });

    it("includes only first-half events for a Half Time pack", () => {
      const input = rapidMatchToIntelligencePackInput(match, "Half Time");
      expect(input.events).toHaveLength(1);
      expect(input.events[0].id).toBe(firstHalfGoal.id);
    });

    it("computes the Half Time score from first-half events only", () => {
      const input = rapidMatchToIntelligencePackInput(match, "Half Time");
      expect(input.homeScore).toEqual({ goals: 1, points: 0, total: 3 });
      expect(input.awayScore).toEqual({ goals: 0, points: 0, total: 0 });
    });

    it("includes every event (both halves) for a Full Time pack", () => {
      const input = rapidMatchToIntelligencePackInput(match, "Full Time");
      expect(input.events).toHaveLength(2);
      expect(input.homeScore).toEqual({ goals: 1, points: 0, total: 3 });
      expect(input.awayScore).toEqual({ goals: 0, points: 1, total: 1 });
