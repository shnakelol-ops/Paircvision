...
export default function TacticalPlaySurface() {
  ...
  const removePlayersById = (removedIds: Set<string>) => {
    ...
    setShotEvents((prev) => prev.filter((s) => !removedIds.has(s.shooterId)));
  ...
  const onResetBoard = () => {
    const shell = shellRef.current;
    if (!shell) return;
    const confirmed = window.confirm("Reset the Tactical Play board? This clears the current board only. Saved scenarios are not deleted.");
    if (!confirmed) return;
        ...
        {itemsOpen && !modeIsPlaybackLocked ? (
          <div style={MOVEMENT_PANEL_STYLE}>
            ...
            <div style={MP_ROW}>
              ...
              <button
                type="button"
                style={trainingItems.length === 0 ? { ...MP_CHIP, opacity: 0.45, cursor: "not-allowed" } : { ...MP_CHIP, color: "rgba(255, 140, 140, 0.75)" }}
                disabled={trainingItems.length === 0}
                onClick={onClearAllTrainingItems}
              >
                Clear All
              </button>
            </div>
