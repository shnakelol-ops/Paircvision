...
// ─── Sport-aware restart terminology ─────────────────────────────────────────
// isPuck(sport) → true for hurling and camogie; false for gaelic/football.
// Use these helpers in every user-facing label that mentions kickouts/puckouts.

function isPuck(sport: PitchSport): boolean {
  return sport === "hurling" || sport === "camogie";
}

/** "Kickout" or "Puckout" (title case) */
function koLabel(sport: PitchSport): string {
  return isPuck(sport) ? "Puckout" : "Kickout";
}

/** "kickout" or "puckout" (lower case) */
function koLabelLC(sport: PitchSport): string {
  return isPuck(sport) ? "puckout" : "kickout";
}

/** "Kickouts" or "Puckouts" (title case, plural) */
function koLabelPlural(sport: PitchSport): string {
  return isPuck(sport) ? "Puckouts" : "Kickouts";
}

/** "kickouts" or "puckouts" (lower case, plural) */
function koLabelPluralLC(sport: PitchSport): string {
  return isPuck(sport) ? "puckouts" : "kickouts";
}

/** "KICKOUT" or "PUCKOUT" (all caps) */
function koLabelUC(sport: PitchSport): string {
  return isPuck(sport) ? "PUCKOUT" : "KICKOUT";
}

/** "KICKOUTS" or "PUCKOUTS" (all caps, plural) */
function koLabelPluralUC(sport: PitchSport): string {
  return isPuck(sport) ? "PUCKOUTS" : "KICKOUTS";
}

/**
 * Replaces "kickout(s)" with "puckout(s)" in a string when sport is hurling/camogie.
 * Preserves original case (lower / Title / UPPER).
 */
function xRestartStr(s: string, sport: PitchSport): string {
  if (!isPuck(sport)) return s;
  return s.replace(/kickouts?/gi, (m) => {
    const allUp  = m === m.toUpperCase() && m !== m.toLowerCase();
    const capFst = !allUp && m[0] === m[0].toUpperCase() && m[0] !== m[0].toLowerCase();
    const plural = m.toLowerCase().endsWith("s");
    const base   = allUp ? "PUCKOUT" : capFst ? "Puckout" : "puckout";
    return plural ? base + (allUp ? "S" : "s") : base;
  });
}
...
/**
 * Truncates a team name to maxChars for tight label contexts.
 * Never cuts mid-word — always appends "…" so the reader knows the name
 * continues, rather than seeing a plausible-but-wrong partial name.
 */
function truncTeam(name: string, maxChars: number): string {
  if (name.length <= maxChars) return name;
  const cut = name.lastIndexOf(" ", maxChars - 2);
...
function makeSegmentsPage(
  ...
  halves.forEach(({ title, accent, accentBg, period, segs, top }) => {
    ...
    const subLabels = ["Score", "Shots/W", `${koLabel(sport)} W-L`, "Net T/O", "Frees W/C"];
...
function makePlayerPages(
  ...
  const colHdrs = ["#", "Name", "Score", "Shots", "Wides",
                   ...
                   `${koLabel(sport)} Won`, `${koLabel(sport)} Lost`, "Frees Won", "Frees Con", "Actions"];
...
function makeChainSummaryPage(
  ...
  // ── COL 1: Chain Patterns Table ─────────────────────────────────────────────
  {
    ...
    cy = drawColumnHeader(COL1_X, cy, COL_W, truncTeam(homeTeam, 12), truncTeam(awayTeam, 12));
  ...
  // ── COL 3: Scoring Runs detail ──────────────────────────────────────────────
  {
    ...
    ctx.textBaseline = "middle";
    ctx.textAlign = "left";
    ctx.fillText(`${truncTeam(homeTeam, 16)} — Best Run`, COL3_X + 14, cy + 10);
    ctx.restore();
    cy += 20;
    ...
    ctx.textBaseline = "middle";
    ctx.textAlign = "left";
    ctx.fillText(`${truncTeam(awayTeam, 16)} — Best Run`, COL3_X + 14, cy + 10);
    ctx.restore();
    cy += 20;
    ...
    if (runsToShow.length === 0) {
      ...
      runsToShow.forEach((run, i) => {
        const sideLabel = run.teamSide === "FOR" ? truncTeam(homeTeam, 10) : truncTeam(awayTeam, 10);
...
function makeKickoutChainPage(
  ...
  function drawPossessionBar(
    ...
    if (total > 0) {
      ...
      ctx.textBaseline = "middle";
      ctx.textAlign = "center";
      ctx.fillText(`No ${koLabelLC(sport)} data`, barX + barW / 2, cy + barH + 12);
    }
    ctx.restore();
    return cy + barH + 26;
  }
  ...
  // ── COL 1: Overall Restart Possession ────────────────────────────────────────
  {
    drawPanelBg(COL1_X, CONTENT_TOP, COL_W, CONTENT_H, "#22d3ee");
    let cy = drawPanelTitle(COL1_X, CONTENT_TOP, `${koLabel(sport)} Possession Overview`, "#22d3ee");
    ...
    if (ko.total === 0) {
      ...
      ctx.textBaseline = "middle";
      ctx.textAlign = "center";
      ctx.fillText(`No ${koLabelLC(sport)} events recorded`, COL1_X + COL_W / 2, CONTENT_TOP + CONTENT_H / 2);
      ctx.restore();
    } else {
      // Possession bar
      cy += 6;
      cy = drawPossessionBar(COL1_X, cy, COL_W, ko.won, ko.lost, truncTeam(homeTeam, 12), truncTeam(awayTeam, 12));
      cy += 4;

      // Summary
      cy = drawStatRow(COL1_X, cy, COL_W, `Total ${koLabelLC(sport)} events`, String(ko.total), "#e2e8f0", false);
      cy = drawStatRow(COL1_X, cy, COL_W, `${truncTeam(homeTeam, 16)} won`, String(ko.won),            "#22d3ee", true);
  ...
  // ── COL 2: Kickout Type Breakdown (FOR top, OPP bottom) ──────────────────────
  {
    ...
    let cy = drawPanelTitle(COL2_X, PANEL_Y1, `${truncTeam(homeTeam, 18)} — ${koLabel(sport)} Types`, "#7dd3fc");
    ...
    if (forKoWonOut.length + forKoConOut.length === 0) {
      ...
      ctx.textBaseline = "middle";
      ctx.textAlign = "center";
      ctx.fillText(`No ${koLabelPluralLC(sport)} recorded`, COL2_X + COL_W / 2, PANEL_Y1 + HALF_H / 2);
      ctx.restore();
    } else {
      ctx.save();
      ctx.fillStyle = "rgba(255,255,255,0.04)";
      ...
      ctx.textBaseline = "middle";
      ctx.textAlign = "left";
      ctx.fillText(`OWN ${koLabelUC(sport)}S CONCEDED`, COL2_X + 14, cy + 10);
      ctx.restore();
      cy += 20;
    ...
    cy = drawPanelTitle(COL2_X, PANEL_Y2, `${truncTeam(awayTeam, 18)} — ${koLabel(sport)} Types`, "#fb7185");
    ...
    if (oppKoWonOut.length + oppKoConOut.length === 0) {
      ...
      ctx.textBaseline = "middle";
      ctx.textAlign = "center";
      ctx.fillText(`No ${koLabelPluralLC(sport)} recorded`, COL2_X + COL_W / 2, PANEL_Y2 + HALF_H / 2);
      ctx.restore();
    } else {
      ctx.save();
      ctx.fillStyle = "rgba(255,255,255,0.04)";
  ...
  {
    drawPanelBg(COL3_X, CONTENT_TOP, COL_W, CONTENT_H, "#fbbf24");
    let cy = drawPanelTitle(COL3_X, CONTENT_TOP, `${koLabel(sport)} Chain Outcomes`, "#fbbf24");
    ...
    cy = drawStatRow(COL3_X, cy, COL_W, `Avg time to score (${koLabel(sport)})`, avgStr, "#fbbf24", false);
    ...
    if (ko.won + ko.lost > 0) {
      ...
      ctx.restore();
      cy += 14;
      drawPossessionBar(COL3_X, cy, COL_W, ko.won, ko.lost, truncTeam(homeTeam, 10), truncTeam(awayTeam, 10));
    }
  }
...
function makeTurnoverPunishmentPage(
  ...
  // ── COL 1: homeTeam — Attacking from Turnovers ───────────────────────────────
  {
    ...
    let cy = drawPanelTitle(COL1_X, CONTENT_TOP, `${truncTeam(homeTeam, 18)} — Turnover Attack`, "#a78bfa");
  ...
  // ── COL 2: awayTeam — Attacking from Turnovers ───────────────────────────────
  {
    ...
    let cy = drawPanelTitle(COL2_X, CONTENT_TOP, `${truncTeam(awayTeam, 18)} — Turnover Attack`, "#fb7185");
  ...
  // ── COL 3: Comparative + Chain Outcomes + Damage Conceded ────────────────────
  {
    ...
    cy += 4;
    cy = drawStatRow(COL3_X, cy, COL_W, `${truncTeam(homeTeam, 16)} — Won → Score %`, pctStr(forWonToScore, forWonTotal),  "#a78bfa", false);
    cy = drawStatRow(COL3_X, cy, COL_W, `${truncTeam(awayTeam, 16)} — Won → Score %`, pctStr(oppWonToScore, oppWonTotal),  "#fb7185", true);
    cy = drawStatRow(COL3_X, cy, COL_W, `${truncTeam(homeTeam, 16)} — Won → Shot %`,  pctStr(forWonToShotAny, forWonTotal), "#a78bfa", false);
    cy = drawStatRow(COL3_X, cy, COL_W, `${truncTeam(awayTeam, 16)} — Won → Shot %`,  pctStr(oppWonToShotAny, oppWonTotal), "#fb7185", true);
    cy += 10;

    // Chain rule matches
    cy = drawSubHeader(COL3_X, cy, COL_W, "CHAIN RULE MATCHES", "#fbbf24");
    cy = drawStatRow(COL3_X, cy, COL_W, `T/O Won → Score  (${truncTeam(homeTeam, 10)})`,  String(toToScoreFor),  "#a78bfa", false);
    cy = drawStatRow(COL3_X, cy, COL_W, `T/O Won → Score  (${truncTeam(awayTeam, 10)})`,  String(toToScoreOpp),  "#fb7185", true);
    ...
    cy = drawStatRow(COL3_X, cy, COL_W, `${truncTeam(awayTeam, 14)} lost → FOR scored`, withPct(oppLostToForScore, oppLostTotal), "#f97316", false);
    cy = drawStatRow(COL3_X, cy, COL_W, `${truncTeam(awayTeam, 14)} lost → FOR shot`,   withPct(oppLostToForShot,  oppLostTotal), "#fbbf24", true);
    cy += 10;
    ...
    const netStr   = netScore > 0 ? `+${netScore} ${truncTeam(homeTeam, 10)}` :
                     netScore < 0 ? `${netScore} ${truncTeam(awayTeam, 10)}` : "Level";
    ...
    const netShotStr = netShot > 0 ? `+${netShot} ${truncTeam(homeTeam, 10)}` :
                       netShot < 0 ? `${netShot} ${truncTeam(awayTeam, 10)}` : "Level";
...
function makeMomentumRunsPage(
  ...
  function runTeamLabel(run: typeof runs[number]): string {
    return run.teamSide === "FOR" ? truncTeam(homeTeam, 14) : truncTeam(awayTeam, 14);
  }
  ...
  // ── COL 2: Half-by-Half Momentum ──────────────────────────────────────────────
  {
    ...
    if (h1Runs.length === 0) {
      ...
      cy = drawStatRow(COL2_X, cy, COL_W, `${truncTeam(homeTeam, 16)} runs (≥2)`,   String(h1For.length),   "#22d3ee", false);
      cy = drawStatRow(COL2_X, cy, COL_W, `${truncTeam(homeTeam, 16)} run-scores`,  String(h1ForScore),     "#22d3ee", true);
      cy = drawStatRow(COL2_X, cy, COL_W, `${truncTeam(awayTeam, 16)} runs (≥2)`,   String(h1Opp.length),   "#fb7185", false);
      cy = drawStatRow(COL2_X, cy, COL_W, `${truncTeam(awayTeam, 16)} run-scores`,  String(h1OppScore),     "#fb7185", true);
      cy += 6;
      if (h1Longest) {
      ...
      const h1Control = h1ForScore > h1OppScore
        ? truncTeam(homeTeam, 14)
        : h1OppScore > h1ForScore ? truncTeam(awayTeam, 14)
        : "Level";
      ...
      const h1LateStr = h1LateRuns.length === 0
        ...
        : `${truncTeam(homeTeam, 10)}: ${h1LateFor} pts  ${truncTeam(awayTeam, 10)}: ${h1LateOpp} pts`;
    ...
    if (h2Runs.length === 0) {
      ...
      cy = drawStatRow(COL2_X, cy, COL_W, `${truncTeam(homeTeam, 16)} runs (≥2)`,   String(h2For.length),   "#22d3ee", false);
      cy = drawStatRow(COL2_X, cy, COL_W, `${truncTeam(homeTeam, 16)} run-scores`,  String(h2ForScore),     "#22d3ee", true);
      cy = drawStatRow(COL2_X, cy, COL_W, `${truncTeam(awayTeam, 16)} runs (≥2)`,   String(h2Opp.length),   "#fb7185", false);
      cy = drawStatRow(COL2_X, cy, COL_W, `${truncTeam(awayTeam, 16)} run-scores`,  String(h2OppScore),     "#fb7185", true);
      cy += 6;
      if (h2Longest) {
      ...
      const h2Control = h2ForScore > h2OppScore
        ? truncTeam(homeTeam, 14)
        : h2OppScore > h2ForScore ? truncTeam(awayTeam, 14)
        : "Level";
      ...
      const h2LateStr = h2LateRuns.length === 0
        ...
        : `${truncTeam(homeTeam, 10)}: ${h2LateFor} pts  ${truncTeam(awayTeam, 10)}: ${h2LateOpp} pts`;
  ...
  // ── COL 3: Run Quality, Response Timing, Segment Control ─────────────────────
  {
    ...
    if (lrf) {
      cy = drawStatRow(COL3_X, cy, COL_W,
        `${truncTeam(homeTeam, 16)} longest`,
        `×${lrf.count}  ${runTimeLabel(lrf)}  ${fmtScore(scoreFromEvents(lrf.events))}`,
        "#22d3ee", false,
      );
    } else {
      cy = drawStatRow(COL3_X, cy, COL_W, `${truncTeam(homeTeam, 16)} longest`, "No run ≥2", "#64748b", false);
    }
    if (lro) {
      cy = drawStatRow(COL3_X, cy, COL_W,
        `${truncTeam(awayTeam, 16)} longest`,
        `×${lro.count}  ${runTimeLabel(lro)}  ${fmtScore(scoreFromEvents(lro.events))}`,
        "#fb7185", true,
      );
    } else {
      cy = drawStatRow(COL3_X, cy, COL_W, `${truncTeam(awayTeam, 16)} longest`, "No run ≥2", "#64748b", true);
    }

    cy = drawStatRow(COL3_X, cy, COL_W, `${truncTeam(homeTeam, 16)} total runs`,        String(forRunTotal),  "#22d3ee", false);
    cy = drawStatRow(COL3_X, cy, COL_W, `${truncTeam(awayTeam, 16)} total runs`,        String(oppRunTotal),  "#fb7185", true);
    cy = drawStatRow(COL3_X, cy, COL_W, `${truncTeam(homeTeam, 16)} total run-scores`,  String(forRunScores), "#22d3ee", false);
    cy = drawStatRow(COL3_X, cy, COL_W, `${truncTeam(awayTeam, 16)} total run-scores`,  String(oppRunScores), "#fb7185", true);

    const netColor  = netRunScores > 0 ? "#4ade80" : netRunScores < 0 ? "#fb7185" : "#94a3b8";
    const netStr    = netRunScores > 0 ? `+${netRunScores} ${truncTeam(homeTeam, 10)}`
                    : netRunScores < 0 ? `${netRunScores} ${truncTeam(awayTeam, 10)}`
                    : "Level";
    ...
    ([1, 2, 3, 4, 5, 6] as const).forEach((seg, i) => {
      ...
      if (total === 0) {
        ...
        controlStr  = `${truncTeam(homeTeam, 10)} +${forScore - oppScore}`;
        controlColor = "#22d3ee";
      } else if (oppScore > forScore) {
        controlStr  = `${truncTeam(awayTeam, 10)} +${oppScore - forScore}`;
        controlColor = "#fb7185";
      } else {
        controlStr  = "Level";
        controlColor = "#94a3b8";
...
function makeTacticalIntelligencePage(
  ...
  if (tvTotal > 0) {
    if (tvConvPct >= 35) {
      insights.push({ text: `${truncTeam(homeTeam, 16)} converted ${tvConvPct}% of won turnovers to scores.` });
    } else if (tvConvPct < 20 && tvWon > 0) {
      insights.push({ text: `${truncTeam(homeTeam, 16)} won ${tvWon}/${tvTotal} turnovers but converted only ${tvConvPct}% to scores.` });
    } else {
      insights.push({ text: `${truncTeam(homeTeam, 16)} won ${tvWinPct}% of turnovers (${tvConvPct}% to scores). ${truncTeam(awayTeam, 16)} scored from ${tvDefExp}% of theirs.` });
    }
  }
  ...
  rcy = drawMetricRow(R_COL_X, rcy, R_COL_W,
    `${truncTeam(homeTeam, 22)} longest burst`,
    longestFor > 0 ? `${longestFor} scores` : "—", "#22d3ee", false);
  rcy = drawMetricRow(R_COL_X, rcy, R_COL_W,
    `${truncTeam(awayTeam, 22)} longest burst`,
    longestOpp > 0 ? `${longestOpp} scores` : "—", "#fb7185", true);
  rcy = drawMetricRow(R_COL_X, rcy, R_COL_W,
    `${truncTeam(homeTeam, 22)} max consecutive`, `${maxConsFor}`, "#22d3ee", false);
  drawMetricRow(R_COL_X, rcy, R_COL_W,
    `${truncTeam(awayTeam, 22)} max consecutive`, `${maxConsOpp}`, "#fb7185", true);
  ...
  rcy = drawMetricRow(R_COL_X, rcy, R_COL_W, `${koLabelPlural(sport)} leading to scores`, `${koToScore}`, "#22d3ee", false);
  ...
  function drawTimeline(colX: number, items: SwingEvent[], emptyText: string): void {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const badge =
        ...
        : s.kind === "KICKOUT_CLUSTER"  ? koLabelLC(sport)
...
function makeTurnoverVisualPage(
  ...
  // ── Panel 1: Turnover Summary ─────────────────────────────────────────────
  {
    ...
    cy = dpMiniBarRow(ctx, DP_P1_X, cy, DP_PANEL_W, `${truncTeam(homeTeam, 14)} Won`, String(forWonTotal), totalTO > 0 ? forWonTotal / totalTO : 0, "#a78bfa", "#a78bfa", false);
    cy = dpMiniBarRow(ctx, DP_P1_X, cy, DP_PANEL_W, `${truncTeam(awayTeam, 14)} Won`, String(oppWonTotal), totalTO > 0 ? oppWonTotal / totalTO : 0, "#fb7185", "#fb7185", true);
...
function makeFreeAnalysisPage(
  ...
  // ── Panel 1: Free Summary ─────────────────────────────────────────────────
  {
    ...
    cy = dpMiniBarRow(ctx, DP_P1_X, cy, DP_PANEL_W, `${truncTeam(homeTeam, 14)} Frees Won`, String(forFreesWon), totalFreesWon > 0 ? forFreesWon / totalFreesWon : 0, "#818cf8", "#818cf8", false);
...
function makeHtKickoutVisionPage(
  ...
  fillDarkBg(ctx);
  drawTopAccentBar(ctx);
  drawPageHeader(ctx, isPuck(sport) ? "Puckout Vision" : "Kickout Vision", `${homeTeam} v ${awayTeam}`, pageNum, totalPages);
...
function makeHtAttackShotVisionPage(
  ...
  const colors: string[] = [];
  if (totalShots > 0) {
    facts.push(`${truncTeam(homeTeam, 12)} shots: ${totalScores} scored · ${totalWides} wide (${shotEff}%)`);
    colors.push("#34d399");
  }
  if (totalOppShots > 0) {
    facts.push(`${truncTeam(awayTeam, 12)} shots: ${totalOppScores} scored · ${totalOppWides} wide (${oppShotEff}%)`);
    colors.push("#ef4444");
  }
  if (scoreHot && facts.length < 3) {
    facts.push(`${truncTeam(homeTeam, 12)} top zone: ${scoreHot.label} (${scoreHot.count})`);
    colors.push("#34d399");
  }
...
function makeHtGameFlowPage(
  ...
  const facts: string[] = [];
  if (forCtrl > 0)     facts.push(`${truncTeam(homeTeam, 14)} controlled ${forCtrl} segment${forCtrl !== 1 ? "s" : ""}`);
  if (oppCtrl > 0)     facts.push(`${truncTeam(awayTeam, 14)} controlled ${oppCtrl} segment${oppCtrl !== 1 ? "s" : ""}`);
...
function makeHtGameFlowFactorsPage(
  ...
  // ── Bottom callout strip ───────────────────────────────────────────────────
  const facts: string[] = [];
  if (forFacts.length > 0)     facts.push(`${truncTeam(homeTeam, 14)}: ${forFacts.length} positive factor${forFacts.length !== 1 ? "s" : ""} identified`);
  if (againstFacts.length > 0) facts.push(`${truncTeam(homeTeam, 14)}: ${againstFacts.length} challenge${againstFacts.length !== 1 ? "s" : ""} flagged`);
  ...
  if (patterns.length === 0) {
    ...
    for (let i = 0; i < patterns.length; i++) {
      ...
      // Badge chip
      {
        ...
        const displayBadge = xRestartStr(p.badge, sport);
        const chipTW = ctx.measureText(displayBadge).width;
        ...
        ctx.textBaseline = "middle";
        ctx.textAlign    = "left";
        ctx.fillText(displayBadge, BX + 9, HDR_MID);
        ctx.restore();
      }
      ...
      ctx.textBaseline = "alphabetic";
      ctx.textAlign    = "left";
      ctx.fillText(xRestartStr(p.headline, sport), IX, HL_Y);
      ctx.restore();
      ...
      ctx.textBaseline = "alphabetic";
      ctx.textAlign    = "left";
      let obsText = xRestartStr(p.observation, sport);
      ...
      ctx.fillStyle = "#94a3b8";
      ctx.textBaseline = "alphabetic";
      ctx.fillText(xRestartStr(p.metricLabel, sport), IX + numW + 12, MET_Y - 10);
      ctx.restore();
...
function makeOurShotProfilePage(
  ...
  if (totalShots > 0) {
    facts.push(`${truncTeam(homeTeam, 12)} shots: ${totalScores} scored from ${totalShots} attempts · ${totalMisses} missed (${shotEff}%)`);
    colors.push("#34d399");
  }
  if (forFreeScored > 0 || forFreeMissed > 0) {
    ...
    facts.push(`${truncTeam(homeTeam, 12)}: ${parts.join(" · ")}`);
    colors.push("#818cf8");
  } else if (freeWonEvts.length > 0 && facts.length < 2) {
    facts.push(`${truncTeam(homeTeam, 12)} won ${freeWonEvts.length} free${freeWonEvts.length !== 1 ? "s" : ""}`);
    colors.push("#818cf8");
  }
  if (scoreHot && facts.length < 3) {
...
function makeOppShotProfilePage(
  ...
  if (totalOppShots > 0) {
    facts.push(`${truncTeam(awayTeam, 12)} shots: ${totalOppScores} scored from ${totalOppShots} attempts · ${totalOppMisses} missed (${oppShotEff}%)`);
    colors.push("#ef4444");
  }
  if (oppFreeScored > 0 || oppFreeMissed > 0) {
    ...
    facts.push(`${truncTeam(awayTeam, 12)}: ${parts.join(" · ")}`);
    colors.push("#f472b6");
  } else if (freeConcededEvts.length > 0 && facts.length < 2) {
    facts.push(`${truncTeam(homeTeam, 12)} conceded ${freeConcededEvts.length} free${freeConcededEvts.length !== 1 ? "s" : ""}`);
    colors.push("#f472b6");
  }
...
// ─── p.3 Restart Battle ───────────────────────────────────────────────────────
/**
 * Restart Battle — visual comparison of restart locations by team.
 *
 * HT: two side-by-side mini pitches (FOR left, OPP right) for 1st half.
 * FT: 2×2 grid — 1st half top row, 2nd half bottom row; FOR left, OPP right.
 *
...
function makeRestartBattlePage(
  ...
  const restartTerm   = isPuck(sport) ? "Puckout" : "Kickout";
  const restartTermLC = isPuck(sport) ? "puckouts" : "kickouts";

  fillDarkBg(ctx);
  drawTopAccentBar(ctx);
  const pageTitle = mode === "HT" ? "Restart Battle – First Half" : "Restart Battle – Full Time";
  drawPageHeader(ctx, pageTitle, `${homeTeam} v ${awayTeam}`, pageNum, totalPages);
  ...
  if (mode === "HT") {
    // ── HT: two side-by-side mini pitches (1st Half) ─────────────────────
    ...
    const homeDirArrow = homeAttackingDirection === "RIGHT" ? "Attacking →" : "← Attacking";
    const awayDirArrow = homeAttackingDirection === "RIGHT" ? "← Attacking" : "Attacking →";
    ...
    ctx.fillText("1st Half", leftCX, BAND_Y + 15);
    ...
    ctx.font = "13px sans-serif"; ctx.fillStyle = "#64748b";
    ctx.fillText("1st Half", rightCX, BAND_Y + 15);
    ...
    // ── FT: 2×2 grid — 1st Half top row · 2nd Half bottom row ───────────
    const splitRestarts = (evts: readonly PdfExportEvent[]) => {
      const home = evts.filter((e) => PDF_KIND_SETS.KICKOUTS.has(e.kind) && getOwner(e) === "FOR");
      const away = evts.filter((e) => PDF_KIND_SETS.KICKOUTS.has(e.kind) && getOwner(e) === "OPP");
      return {
        homeRestarts: home,
        awayRestarts: away,
        homeRetained: home.filter((e) => getWinner(e) === "FOR"),
        homeLost:     home.filter((e) => getWinner(e) !== "FOR"),
        awayRetained: away.filter((e) => getWinner(e) === "OPP"),
        awayLost:     away.filter((e) => getWinner(e) !== "OPP"),
      };
    };

    const r1 = splitRestarts(events.filter((e) => e.period === "1H"));
    const r2 = splitRestarts(events.filter((e) => e.period === "2H"));

    // Layout: two rows, each row = band + pitch + stat label
    // Total available: BAND_Y(80) → HT_STRIP_TOP(900) = 820px
    // Per row: BAND_H(50) + 4 + PITCH_H + STAT_H(32); two rows + ROW_GAP(20) between
    // PITCH_H = (820 - 2*(50+4+32) - 20) / 2 = (820 - 172 - 20) / 2 = 314
    const BAND_H   = 50;
    const BAND_Y   = 80;
    const STAT_H   = 32;
    const ROW_GAP  = 20;
    const PITCH_H  = (HT_STRIP_TOP - BAND_Y - ROW_GAP - 2 * (BAND_H + 4 + STAT_H)) / 2; // 314

    const ROW1_BAND_Y  = BAND_Y;
    const ROW1_PITCH_Y = ROW1_BAND_Y + BAND_H + 4;
    const ROW1_STAT_Y  = ROW1_PITCH_Y + PITCH_H;
    const ROW2_BAND_Y  = ROW1_STAT_Y + STAT_H + ROW_GAP;
    const ROW2_PITCH_Y = ROW2_BAND_Y + BAND_H + 4;
    const ROW2_STAT_Y  = ROW2_PITCH_Y + PITCH_H;

    const leftX  = GAP_H;
    const rightX = GAP_H + HALF_W + GAP_H;

    // Attacking direction flips between halves (teams swap ends at half-time)
    const h1HomeDirArrow = homeAttackingDirection === "RIGHT" ? "Attacking →" : "← Attacking";
    const h1AwayDirArrow = homeAttackingDirection === "RIGHT" ? "← Attacking" : "Attacking →";
    const h2HomeDirArrow = h1AwayDirArrow;
    const h2AwayDirArrow = h1HomeDirArrow;

    const drawBand = (
      bx: number, by: number, bw: number,
      halfLabel: string, teamLabel: string, dirArrow: string,
      bgColor: string, strokeColor: string, textColor: string,
    ) => {
      ctx.save();
      ctx.fillStyle = bgColor;
      ctx.fillRect(bx, by, bw, BAND_H);
      ctx.strokeStyle = strokeColor;
      ctx.lineWidth   = 1.5;
      ctx.strokeRect(bx + 0.75, by + 0.75, bw - 1.5, BAND_H - 1.5);
      ctx.textAlign    = "center";
      ctx.textBaseline = "middle";
      const cx = bx + bw / 2;
      ctx.font = "12px sans-serif"; ctx.fillStyle = "#64748b";
      ctx.fillText(halfLabel, cx, by + 12);
      ctx.font = "bold 16px sans-serif"; ctx.fillStyle = textColor;
      ctx.fillText(teamLabel, cx, by + 30);
      ctx.font = "12px sans-serif"; ctx.fillStyle = "#94a3b8";
      ctx.fillText(dirArrow, cx, by + 45);
      ctx.restore();
    };

    const HOME_BG     = "rgba(34,197,94,0.12)";
    const HOME_STROKE = "rgba(34,197,94,0.45)";
    const HOME_TEXT   = "#4ade80";
    const AWAY_BG     = "rgba(239,68,68,0.12)";
    const AWAY_STROKE = "rgba(239,68,68,0.45)";
    const AWAY_TEXT   = "#f87171";

    drawBand(leftX,  ROW1_BAND_Y, HALF_W, "1st Half", `${truncTeam(homeTeam, 20)} ${restartTerm}s`, h1HomeDirArrow, HOME_BG, HOME_STROKE, HOME_TEXT);
    drawBand(rightX, ROW1_BAND_Y, HALF_W, "1st Half", `${truncTeam(awayTeam, 20)} ${restartTerm}s`, h1AwayDirArrow, AWAY_BG, AWAY_STROKE, AWAY_TEXT);
    drawBand(leftX,  ROW2_BAND_Y, HALF_W, "2nd Half", `${truncTeam(homeTeam, 20)} ${restartTerm}s`, h2HomeDirArrow, HOME_BG, HOME_STROKE, HOME_TEXT);
    drawBand(rightX, ROW2_BAND_Y, HALF_W, "2nd Half", `${truncTeam(awayTeam, 20)} ${restartTerm}s`, h2AwayDirArrow, AWAY_BG, AWAY_STROKE, AWAY_TEXT);

    const innerTL = renderPitch(ctx, sport, { x: leftX,  y: ROW1_PITCH_Y, w: HALF_W, h: PITCH_H });
    const innerTR = renderPitch(ctx, sport, { x: rightX, y: ROW1_PITCH_Y, w: HALF_W, h: PITCH_H });
    const innerBL = renderPitch(ctx, sport, { x: leftX,  y: ROW2_PITCH_Y, w: HALF_W, h: PITCH_H });
    const innerBR = renderPitch(ctx, sport, { x: rightX, y: ROW2_PITCH_Y, w: HALF_W, h: PITCH_H });

    drawRetained(innerTL, r1.homeRetained); drawLost(innerTL, r1.homeLost);
    drawRetained(innerTR, r1.awayRetained); drawLost(innerTR, r1.awayLost);
    drawRetained(innerBL, r2.homeRetained); drawLost(innerBL, r2.homeLost);
    drawRetained(innerBR, r2.awayRetained); drawLost(innerBR, r2.awayLost);

    // ── Inline stat labels (Retained X/Y %) ──────────────────────────────
    const drawStat = (bx: number, statY: number, bw: number, team: string, retained: number, total: number) => {
      const pct = total > 0 ? Math.round((retained / total) * 100) : 0;
      const txt = total > 0
        ? `${truncTeam(team, 14)} retained ${retained}/${total} (${pct}%)`
        : `No ${restartTermLC} logged`;
      ctx.save();
      ctx.textAlign    = "center";
      ctx.textBaseline = "middle";
      ctx.font      = "22px sans-serif";
      ctx.fillStyle = "#e2e8f0";
      ctx.fillText(txt, bx + bw / 2, statY + STAT_H / 2);
      ctx.restore();
    };

    drawStat(leftX,  ROW1_STAT_Y, HALF_W, homeTeam, r1.homeRetained.length, r1.homeRestarts.length);
    drawStat(rightX, ROW1_STAT_Y, HALF_W, awayTeam, r1.awayRetained.length, r1.awayRestarts.length);
    drawStat(leftX,  ROW2_STAT_Y, HALF_W, homeTeam, r2.homeRetained.length, r2.homeRestarts.length);
    drawStat(rightX, ROW2_STAT_Y, HALF_W, awayTeam, r2.awayRetained.length, r2.awayRestarts.length);

    const kickoutCount = events.filter((e) => PDF_KIND_SETS.KICKOUTS.has(e.kind)).length;
    drawEventCountFooter(ctx, kickoutCount);
  }
