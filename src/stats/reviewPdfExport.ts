...
export type ReviewPdfExportInput = {
  ...
  /**
   * First-half attacking direction for the home (FOR) team. Drives the
   * "← Attacking" / "Attacking →" arrows on the Restart Battle header AND
   * (see zones/zone-orientation.ts) which physical end each event's own
   * team+half combination counts as "Attacking" vs "Defensive" for every
   * zone label/hotspot in this report. Defaults to "RIGHT".
   */
...
function makeKickoutChainPage(
  ...
  // ── COL 1: Overall Restart Possession ────────────────────────────────────────
  {
    ...
    if (ko.total === 0) {
      ...
      cy = drawStatRow(COL1_X, cy, COL_W, "1H — Won → Score (origin)",   String(h1WonToScore),   "#4ade80", false);
      ...
      cy = drawStatRow(COL1_X, cy, COL_W, "2H — Won → Score (origin)",   String(h2WonToScore),   "#4ade80", true);
      cy += 8;
...
function makeTurnoverPunishmentPage(
  ...
  // ── COL 1: homeTeam — Attacking from Turnovers ───────────────────────────────
  {
    ...
    if (forWonTotal === 0) {
      ...
      cy = drawStatRow(COL1_X, cy, COL_W, "→ Origin score",    withPct(forWonToScore,   forWonTotal),  "#4ade80", true);
  ...
  // ── COL 2: awayTeam — Attacking from Turnovers ───────────────────────────────
  {
    ...
    if (oppWonTotal === 0) {
      ...
      cy = drawStatRow(COL2_X, cy, COL_W, "→ Origin score",    withPct(oppWonToScore,   oppWonTotal),  "#4ade80", true);
  ...
  // ── COL 3: Comparative + Chain Outcomes + Damage Conceded ────────────────────
  {
    ...
    cy += 4;
    cy = drawStatRow(COL3_X, cy, COL_W, `${truncTeam(homeTeam, 16)} — Won → Origin score %`, pctStr(forWonToScore, forWonTotal),  "#a78bfa", false);
    cy = drawStatRow(COL3_X, cy, COL_W, `${truncTeam(awayTeam, 16)} — Won → Origin score %`, pctStr(oppWonToScore, oppWonTotal),  "#fb7185", true);
...
function makeTacticalIntelligencePage(
  ...
  cy = drawMetricRow(L_COL_X, cy, L_COL_W, `${koLabel(sport)} wins → origin score`,             `${koConvPct}%`, "#34d399", false);
  cy = drawMetricRow(L_COL_X, cy, L_COL_W, `${koLabel(sport)} losses → origin score against`,   `${koExpPct}%`, "#fb7185", true);
  {
  ...
  cy = drawMetricRow(L_COL_X, cy, L_COL_W, "Turnover wins → shot, no score",   `${tvShotOnly}%`, "#fbbf24", true);
  drawMetricRow(L_COL_X, cy, L_COL_W, "Turnover losses → origin score against", `${tvDefExp}%`, "#fb7185", false);
...
function makeZoneAnalysisPage(
  ...
  // Team-relative: FOR and OPP subsets each resolve their own effective
  // attacking direction per event (teamSide + period) — see zone-orientation.ts.
...
function makeHowToReadPage(
  ...
  // ── Footer disclaimers — canonical restart vocabulary ─────────────────────
  ctx.fillStyle = "#475569";
  ctx.font = "italic 16px sans-serif";
  ctx.textAlign = "center";
  ctx.fillText(restartAttributionFootnote(sport), CANVAS_W / 2, CANVAS_H - 58);
  ctx.fillText(restartExplainerLine(sport), CANVAS_W / 2, CANVAS_H - 36);
...
function makeRestartVisualPage(
  ...
  // ── Panel 2: Chain Outcomes ───────────────────────────────────────────────
  {
    ...
    cy = dpMiniBarRow(ctx, DP_P2_X, cy, DP_PANEL_W, "Won → Score (origin)",        withPct(forScoredFromKo, forWonTotal), forWonTotal > 0 ? forScoredFromKo / forWonTotal : 0, "#4ade80", "#4ade80", false);
    ...
    cy = dpMiniBarRow(ctx, DP_P2_X, cy, DP_PANEL_W, "Lost → Score against (origin)", withPct(oppScoredFromKo, oppWonTotal), oppWonTotal > 0 ? oppScoredFromKo / oppWonTotal : 0, "#f97316", "#f97316", false);
...
function makeTurnoverVisualPage(
  ...
  // ── Panel 2: Consequences ─────────────────────────────────────────────────
  {
    ...
    cy = dpMiniBarRow(ctx, DP_P2_X, cy, DP_PANEL_W, "Won → Score (origin)",        withPct(forWonToScore, forWonTotal), forWonTotal > 0 ? forWonToScore / forWonTotal : 0, "#4ade80", "#4ade80", false);
    ...
    cy = dpMiniBarRow(ctx, DP_P2_X, cy, DP_PANEL_W, "Lost → Score against (origin)", withPct(forLostToOppScore, forLostTotal), forLostTotal > 0 ? forLostToOppScore / forLostTotal : 0, "#f97316", "#f97316", false);
    ...
    cy = dpMiniBarRow(ctx, DP_P2_X, cy, DP_PANEL_W, "Won → Score (origin)",        withPct(oppWonToScore, oppWonTotal), oppWonTotal > 0 ? oppWonToScore / oppWonTotal : 0, "#fb7185", "#fb7185", false);
...
// Team-relative: each event's own teamSide + period resolves its effective
// attacking direction against `homeAttackingDirection` (the FOR team's H1
// direction) — see zones/zone-orientation.ts. This is what makes "Attacking"
// / "Defensive" zone labels correct regardless of which end a team is
// shooting at, or which half the event happened in.
function pdfZoneCounts(evts: readonly PdfExportEvent[], homeAttackingDirection: AttackingDirection) {
  return getTeamRelativeZoneCounts(
    evts as unknown as Parameters<typeof getTeamRelativeZoneCounts>[0],
    homeAttackingDirection,
  );
}
function pdfZoneHotspots(evts: readonly PdfExportEvent[], homeAttackingDirection: AttackingDirection) {
...
function makeOppShotProfilePage(
  ...
  // ── Zone counts (for headline + hotspot highlight) ────────────────────────
  const scoreCounts = pdfZoneCounts(oppScoreEvts, homeAttackingDirection);
...
export async function exportSnapshotPdf(input: SnapshotPdfExportInput): Promise<void> {
  ...
  if (isHT) {
    ...
    addPage(
      makeTurnoverTerritoryPage(events, chainAnalysis, sport, home, away, 4, TOTAL_PAGES, homeAttackingDirection),
      true,
      "Turnover & Territory",
      "POSSESSION",
    );
    ...
    addPage(
      makeTurnoverTerritoryPage(events, chainAnalysis, sport, home, away, 7, TOTAL_PAGES, homeAttackingDirection),
      true,
      "Turnover & Territory",
      "POSSESSION",
    );
